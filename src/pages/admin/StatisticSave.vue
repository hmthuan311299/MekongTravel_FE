...
export default {
    ...
    props:{
      statisticTitle: {type: String, default:'Thống kê địa điểm được lưu lại nhiều nhất'}
    },
    data(){
        return{
            provinceId:'',
    ...
    methods:{
        ...
        ...mapActions(['statisticSave', 'getProvince']),
        getStatisticSave(provinceId, year, month){
            this.statisticSave({provinceId, year, month}).then(response=>{
                if(response.ok){
                    this.labels = [];
        ...
        handleGetMonth(monthId, monthTitle){
            ...
            if(this.year){
                this.getStatisticSave(this.provinceId, this.year, this.month)
            }else{
